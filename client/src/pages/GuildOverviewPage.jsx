...
const GuildOverviewPage = () => {
  ...
  const initialContacts = [
    { id: 1, type: 'Discord', label: 'Discord Invite Link', value: 'discord.gg/designmasters', placeholder: 'Enter Discord link' },
    { id: 2, type: 'Twitter', label: 'Twitter/X Handle', value: '@DesignMasters', placeholder: 'Enter Twitter handle' },
    { id: 3, type: 'Website', label: 'Website URL', value: 'designmasters.community', placeholder: 'Enter website URL' },
  ];
  const [guildContactsList, setGuildContactsList] = useState(initialContacts);
  const availableContactTypes = ['Website', 'Email', 'Discord', 'Twitter', 'Bluesky', 'Twitch'];

  // Handlers for dynamic contacts
  const handleContactChange = (id, newValue) => {
    setGuildContactsList(prevContacts => 
      prevContacts.map(contact => contact.id === id ? { ...contact, value: newValue } : contact)
    );
  };
  ...
  const handleAddContact = (type) => {
    const newContact = {
      ...
      type: type,
      label: `${type} Link/Handle`,
      value: '',
      placeholder: `Enter ${type} details`
    };
    setGuildContactsList(prevContacts => [...prevContacts, newContact]);
    setShowAddContactDropdown(false); // Close dropdown after adding
  };

  const handleRemoveContact = (id) => {
    setGuildContactsList(prevContacts => prevContacts.filter(contact => contact.id !== id));
  };
          ...
          <div className="modal">
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => {
                setEditContactsModalOpen(false);
                // Optionally reset contacts to initial or last saved state on close
                // setGuildContactsList(initialContacts); 
              }}>
                <X size={24} />
              </button>
            </div>
            <div className="modal-body">
              {guildContactsList.map(contact => (
                <div className="form-group" key={contact.id}>
                  <label className="form-label" htmlFor={`contact-${contact.id}`}>{contact.label}</label>
              ...
              <div className="form-group" style={{ position: 'relative' }}>
                ...
                {showAddContactDropdown && (
                  <div className="contact-type-dropdown" style={{ position: 'absolute', top: '100%', left: 0, zIndex: 10 }}>
                    {availableContactTypes.map(type => (
                      <div 
                        key={type} 
                        className="contact-type-item" 
            ...
            <div className="modal-footer">
              <button className="button button-secondary" onClick={() => {
                setEditContactsModalOpen(false);
                // Optionally reset contacts 
                // setGuildContactsList(initialContacts); 
              }}>Cancel</button>
              <button className="button button-primary" onClick={() => setEditContactsModalOpen(false) /* TODO: Save changes */}>Save Changes</button>
            </div>
          </div>
        </div>
      )}
      ...
      <style jsx>{`
        ...
        * {
        ...
        .accordion-content.expanded {
          max-height: 2000px; /* Adjusted for potentially long tables */
          opacity: 1;
          padding-top: 1.5rem;
        }
        ...
        .badge-circle-inner img {
          width: 100%;
          height: 100%;
          object-fit: cover;
        }

        /* Star badge using SVG for consistent border */
        .badge-star-border {
          position: absolute;
          width: 100%;
          height: 100%;
          background-color: var(--silver);
          clip-path: polygon(79.39% 90.45%, 50% 80%, 20.61% 90.45%, 21.47% 59.27%, 2.45% 34.55%, 32.37% 25.73%, 50% 0%, 67.63% 25.73%, 97.55% 34.55%, 78.53% 59.27%);
        }
        ...
        .badge-star-inner img {
          width: 100%;
          height: 100%;
          object-fit: cover;
        }

        /* Heart badge using SVG path for more control */
        .badge-heart-border {
          position: absolute;
          width: 100%;
          height: 100%;
